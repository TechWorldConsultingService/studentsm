...
const AttendancePrincipal = () => {
  ...
  // Delete all attendance for selected class & date
...
<div className="bg-gray-100 p-6 min-h-screen flex justify-center items-center">
      <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-4xl">
        ...
        <p className="text-gray-600 text-center mt-2">Select class and date to view attendance.</p>
        ...
        {/* Date Picker */}
        <div className="mt-4">
          <label className="block text-gray-700 text-lg font-medium">Select Date</label>
          <input
            type="date"
            value={selectedDate}
        ...
        {attendanceRecords?.attendance?.length > 0 && (
          <div className="mt-6 p-4 bg-purple-100 rounded-lg">
            ...
            <p className="text-gray-700 mt-2">Date: <span className="font-semibold">{attendanceRecords.date}</span></p>
            <p className="text-gray-700 mt-2">Class: <span className="font-semibold">{attendanceRecords?.class?.name}</span></p>
            <p className="text-gray-700 mt-2">Total Students: <span className="font-semibold">{totalStudents}</span></p>
            <p className="text-purple-700">Present: <span className="font-semibold">{presentCount}</span></p>
            <p className="text-red-600">Absent: <span className="font-semibold">{absentCount}</span></p>
          </div>
        )}
          ...
          <div className="fixed inset-0 flex justify-center items-center bg-gray-500 bg-opacity-75 z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 md:w-1/3">
              ...
              <p className="mt-3 text-gray-700">
                Are you sure you want to delete all attendance records for <strong>Class {selectedClass}</strong> on <strong>{selectedDate}</strong>? 
          ...
          <table className="w-full border-collapse border border-purple-300">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="4" className="p-3 text-center text-gray-600">No attendance data available.</td>
                </tr>
              ) : (
