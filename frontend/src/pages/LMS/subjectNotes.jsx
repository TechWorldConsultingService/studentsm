...
const SubjectNotes = () => {
  ...
  console.log(selectedSubject, "selected subject is ")

  useEffect(() => {
    const fetchNotes = async () => {
      if (!access || !selectedSubject) {
        toast.error("Please select a subject first.");
        setErrorNotes("Please select a subject first.");
        return;
      }
      ...
      try {
        ...
        setErrorNotes(fetchedNotes.length ? "" : "No notes found.");
      } catch (error) {
        setNotes([]);
        setErrorNotes(
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 md:w-1/2 max-h-[80vh] overflow-y-auto">
              ...
              <div className="mt-4 space-y-3">
                <p className="text-gray-700">
                  <strong>Class:</strong>{" "}
                  {selectedNote.class_code?.class_name || "N/A"}
                </p>
                <p className="text-gray-700">
                  <strong>Subject:</strong>{" "}
                  {selectedNote.subject?.subject_name || "N/A"}
                </p>
                <p className="text-gray-700">
                  <strong>Chapter:</strong> {selectedNote.chapter}
                </p>
                <p className="text-gray-700">
                ...
                {selectedNote.file ? (
                  <p className="text-blue-600 underline">
                    <a
                      href={`http://localhost:8000${selectedNote.file}`}
                      target="_blank"
                ...
                <p className="text-gray-700 text-sm">
                  <strong>Uploaded By:</strong> {selectedNote.created_by}
                </p>
                <p className="text-gray-700 text-sm">
