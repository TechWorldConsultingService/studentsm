...
const SyllabusModal = ({ isEdit, syllabus, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (access && teacher_id && selectedClassName) {
      fetch(`http://localhost:8000/api/filter-subjects/?teacher=${teacher_id}&class_assigned=${selectedClassName}`)
        ...
        chapters.map((chapter) =>
          chapter.id === chapterId
            ? {
                ...
                topics: [
                  ...chapter.topics,
                  { id: Date.now(), name: topicName, is_completed: false, subtopics: [] },
                ],
              }
            : chapter
        )
  ...
  const updateSyllabus = async (payload) => {
    if (!access) {
      toast.error("User is not authenticated. Please log in.");
      return;
    }
    ...
    try {
      await axios.patch(`http://localhost:8000/api/syllabus/${syllabus.id}/`, payload, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${access}`,
        },
  ...
  return (
    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-lg w-11/12 md:w-2/3 max-h-[90vh] overflow-y-auto">
        <h2 className="text-3xl mb-8 font-extrabold text-purple-800">
          {isEdit ? "Edit Syllabus" : "Add Syllabus"}
        </h2>
          ...
          <div key={chapter.id} className="mb-6 bg-purple-50 p-4 rounded-xl border border-purple-200">
            <div className="flex justify-between items-center mb-3">
              ...
              <div className="space-x-2">
                <button
                  onClick={() =>
                    handleEdit("chapter", chapter.id, prompt("Edit Chapter Name", chapter.name))
                  }
